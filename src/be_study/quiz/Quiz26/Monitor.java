...
public class Monitor {
	...
	int price; //가격
	String color; //색상
	int maxResolutionX; //최대해상도 X  4096   ---> 2048
	int maxResolutionY; //최대해상도 Y  
	
	//생성자 
	//Monitor(){          //  new Monitor();
	//}
	
	//new Monitor("제조사", 40, 101010);
	Monitor(String company, int inch, int price){
		...
		this.color = null;
		this.maxResolutionX = 0;
		this.maxResolutionY = 0;
	}
	...
	// increasePrice    addPrice(30000);
	int addPrice(int p) {  //inputPrice
		this.price = this.price + p; //매개변수로 가격인상 처리
	...
	@ 11. 2) 의 가격을 7만원 인상시키고, 그 결과 총금액을 받아서 출력합니다.-> 6번,9번 메소드활용 (9번은...밑에 12번에서 출력을 다시 하니까...6번만해도..되겠군요)
